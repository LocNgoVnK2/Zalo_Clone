...
import { RenewTokenResetPassword } from "../Services/userService";
import { ValidateResetPassword } from "../Services/userService";
  ...
  useEffect(() => {
    if (token && !isApiCalledRef.current) {
      ...
      const validateToken = async () => {
        try {
          let res = await ValidateResetPassword(token);

          if (res){
            
            localStorage.setItem('tokenRP', token);
          ...
          if (error.response && error.response.status === 400) {
            ...
            await RenewTokenResetPassword(token);
          } else if (error.response && error.response.status === 404)  {
            return;
          }else{
            alert("Đã gặp lỗi j đó");
          }
        }
      };
